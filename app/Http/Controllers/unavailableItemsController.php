...
{
    //paratmeter is from the request class as we collect data from somewhwre
    public function storeUn(Request $request){
        $UnItem = new unavailableItems; //creating an object from the model
        //$UnItem can access all the fields in the table
        // dd($request->all());
        ...
        $data2=unavailableItems::all();  //retrieve all the data relating to this model
       // dd($data);
    ...
    public function DeteleEntry($id){
        $UnItem=unavailableItems::find($id); //find a particular id
        $UnItem->delete();
        return redirect()->back();
    }
    //retruve data inorder to update
